...
def setup_fptree(df, min_support):
    num_itemsets = df.shape[0]  # number of itemsets in the database

    is_sparse = False
    if hasattr(df, "sparse"):
        # DataFrame with SparseArray (pandas >= 0.24)
        if df.size == 0:
            itemsets = df.to_numpy()
        else:
...
def generate_itemsets(generator, num_itemsets, colname_map):
    ...
    res_df = pl.DataFrame({"support": supports, "itemsets": itemsets})
...
def valid_input_check(df):
    ...
    all_bools = True  # df.dtypes.apply(pd.api.types.is_bool_dtype).all()
    if not all_bools:
        warnings.warn(
