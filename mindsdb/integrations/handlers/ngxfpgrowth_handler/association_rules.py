...
def association_rules(df, metric="confidence", min_threshold=0.8, support_only=False):
    ...
    keys = df["itemsets"].to_numpy()
    values = df["support"].to_numpy()
    ...
    if not rule_supports:
        return pl.DataFrame(schema=["antecedents", "consequents"] + columns_ordered)
        ...
        df_res = pl.DataFrame(
            ...
            schema=["antecedents", "consequents"],
        )
        ...
        if support_only:
            ...
            for m in columns_ordered:
                # df_res[m] = np.nan
                df_res = df_res.with_columns(pl.Series(name=m, values=np.nan))
            df_res = df_res.with_columns(pl.Series(name="support", values=sAC))
            # df_res["support"] = sAC
            ...
            for m in columns_ordered:
                ...
                df_res = df_res.with_columns(
                    pl.Series(name=m, values=metric_dict[m](sAC, sA, sC))
                )
