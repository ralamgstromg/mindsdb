...
class TransactionEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, sparse=False, as_df=False):
        ...
        if sparse:
            ...
            if as_df:
                array = pl.DataFrame(array, schema=self.columns_)
        return array
    ...
    def fit_transform(self, X, sparse=False, as_df=False):
        """Fit a TransactionEncoder encoder and transform a dataset."""
        return self.fit(X).transform(X, sparse=sparse, as_df=as_df)
