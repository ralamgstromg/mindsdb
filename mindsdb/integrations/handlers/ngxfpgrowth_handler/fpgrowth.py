...
from .fpcommon import valid_input_check, generate_itemsets, setup_fptree
...
def fpgrowth(df, min_support=0.5, use_colnames=False, max_len=None, verbose=0):
    ...
    minsup = math.ceil(min_support * df.shape[0])  # min support as count
    generator = fpg_step(tree, minsup, colname_map, max_len, verbose)

    return generate_itemsets(generator, df.shape[0], colname_map)
