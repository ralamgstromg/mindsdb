...
class S3NgxHandler(DatabaseHandler):
    ...
    def __init__(self, name: Text, connection_data: Optional[Dict], **kwargs):
        ...
        self.resource: boto3.resource = None
    ...
    def query(self, query: ASTNode) -> Response:
        ...
        if isinstance(query, DropTables):
            ...
            if table_name == 'files':
                ...
                # df = table.list(targets=query.targets, conditions=query.where, limit=query.limit)
                ...
                logger.info(f'Files found: {df.shape}')
    ...
    def get_objects(self, limit=None, buckets=None) -> List[dict]:
        ...
        for bucket in scan_buckets:
            ...
            # limit = limit if limit is not None else 999999999
            # logger.info(f'Listing objects in bucket: {bucket}, limit: {limit}')
            # resp = client.list_objects_v2(Bucket=bucket, MaxKeys=limit)
            # print(len(resp['Contents']))
            #resp = client.objects.filter(Bucket=bucket).limit(limit) if limit is not None else client.objects.filter(Bucket=bucket).all()
            # resp = client.list_objects_v2(Bucket=bucket, MaxKeys=limit) if limit is not None else client.list_objects_v2(Bucket=bucket, MaxKeys=999999999)
            #resp = client.bucket(bucket).objects.all() if limit is None else client.bucket(bucket).objects.limit(limit)
            # logger.info(resp)
            # resp = client.resource('s3').Bucket(bucket).objects.all() if limit is None else client.resource('s3').Bucket(bucket).objects.limit(limit)
            resp = self.resource('s3').bucket(bucket).objects.all()
            print(resp[:10])
            logger.info(f'Listing objects in bucket: {bucket}, limit: {limit}, found: {len(resp)}')
