        ...
        with connection.cursor() as cur:
            try:
                ...
                for column in cur.description:
                    ...
                    match column_type:
                        case 1: # Varchar
                            schema_override[column_name] = pd.String
                            mysql_types.append(MYSQL_DATA_TYPE.TEXT)
                        case 2: # Date
                            schema_override[column_name] = pd.Date
                            mysql_types.append(MYSQL_DATA_TYPE.DATE)
                        case 3: # Integer
                            schema_override[column_name] = pd.Int64
                            mysql_types.append(MYSQL_DATA_TYPE.BIGINT)
                        case 4: # Datetime
                            schema_override[column_name] = pd.Datetime
                            mysql_types.append(MYSQL_DATA_TYPE.DATETIME)
                        case 5: # Decimal
                            schema_override[column_name] = pd.Float64
                            mysql_types.append(MYSQL_DATA_TYPE.FLOAT)
                        case pymssql.BINARY: # Binary
                            schema_override[column_name] = pd.Binary
                            mysql_types.append(MYSQL_DATA_TYPE.BINARY)
                        case _:
                            logger.info(f"Unknown type: {column_type}, use VARCHAR as fallback.")
                            schema_override[column_name] = pd.String
                            mysql_types.append(MYSQL_DATA_TYPE.VARCHAR)

                # 8:30
                result = pd.read_database(sql, connection=cur, schema_overrides=schema_override)

                #result = pd.read_database_uri(query=sql, connection=cur, schema_overrides=schema_override)
                ...
                # # 8:42
                # result = pd.DataFrame(schema=schema_override)
                # #arr_df = []
                # #for df_chunk in pd.read_database(sql, connection=cur, schema_overrides=schema_override, iter_batches=True, batch_size=5000):
                # #    arr_df.append(df_chunk)                    
                # #arr_df = [df_chunk for df_chunk in pd.read_database(sql, connection=cur, schema_overrides=schema_override, iter_batches=True, batch_size=5000)]
                # result = pd.concat([df_chunk for df_chunk in pd.read_database(sql, connection=cur, schema_overrides=schema_override, iter_batches=True, batch_size=15000)])
